...
//
// API "websocket test": public Resource Client
//
// Command:
// $ goagen
...
// DownloadPublic downloads /files with the given filename and writes it to the file dest.
// It returns the number of bytes downloaded in case of success.
func (c *Client) DownloadPublic(ctx context.Context, filename, dest string) (int64, error) {
	scheme := c.Scheme
	if scheme == "" {
		scheme = "http"
	}
	p := path.Join("/public/", filename)
